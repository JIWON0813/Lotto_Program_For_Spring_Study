...
![6-lotto](./image/6-lotto.JPG)

<h4>저번 실패 이유</h4>
디버깅 결과 스크래핑도 제대로 해오고 List에도 제대로 데이터가 담김<br>
C언어였나 리스트의 값을 불러오려면 indexOf를 사용해야해서 당연하게 그걸 사용했는데<br>
자바에서는 구현된 메서드가 조금 달랐음<br>
내부 정의를 읽어보니 잘못 쓰고 있던걸 알게되고 원하는 구현의 메서드를 사용하니 성공<br>
현재 자주사용하는 C#에서는 리스트를 배열처럼 호출할 수 있어서(ex. list[index]) 자바 문법이 헷갈림<br>

번호들이 -1로만 들어갔던 이유가 List 메서드 중 indexOf를 사용해서 -1이 호출됐다<br>
(해당 데이터값이 리스트에 존재하지 않으면 -1 리턴)

![6-indexOf](./image/6-indexOf.JPG)

내가 원하던 메서드

![6-get](./image/6-get.JPG)

현재는 서버를 시작해야 최신화를 하지만<br>
추후 무중단배포(구현할지는 모르겠지만)시에는 시간을 설정해서 자동 최신화를 시킬예정<br>
1차적인 목표는 성공했고 이제 해당 데이터로<br>
내가 생각한 패턴들을 적용해 어느정도 원하는 예상 번호들이 나오도록 만들예정<br>